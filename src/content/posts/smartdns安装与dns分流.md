...
author: Jinx
date: 2025-12-25
lastMod: 2025-12-29T14:30:00.000Z
...
```bash
mkdir -p docker/smartdns/data/etc/smartdns

vim docker/smartdns/data/etc/smartdns/smartdns.conf
```
...
```bash
vim docker/smartdns/docker-compose.yml
```
...
这个组合命令会先解锁文件（如果之前锁过），写入内容，然后加锁（+i 属性）。加锁后，即便是系统服务也无法修改该文件，直到你手动解锁。

```shell
chattr -i /etc/resolv.conf 2>/dev/null; echo -e "# Generated by resolvconf\nnameserver 1.2.3.4" > /etc/resolv.conf && chattr +i /etc/resolv.conf
```
